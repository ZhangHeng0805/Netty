...
class ChatServerHandler extends SimpleChannelInboundHandler<String> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        channelGroup.writeAndFlush(sdf.format(new Date())+"--[客户端]"+channel.remoteAddress()
                +"上线了\n当前在线人数："+channelGroup.size()+"\n");
        //将当前channel加入到channelGroup中
//        channelGroup.add(channel);
        System.out.println(sdf.format(new Date())+"--[客户端]"+channel.remoteAddress()
                +"上线了\n当前在线人数："+channelGroup.size()+"\n");
    }
...
//        cause.printStackTrace();
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, String s) throws Exception {
        ...
        for (Channel ch : channelGroup) {
            if (channel!=ch){
                ch.writeAndFlush(sdf.format(new Date())+"--["+channel.remoteAddress()+"]："+s+"\n");
            }else {
